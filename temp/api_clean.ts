...
const ApiService = {
  ...
  // Toggle student status (protected)
  toggleStudentStatus: async (id: number): Promise<ApiResponse<any>> => {
    try {
      const token = getToken();
      if (!token) {
        return { error: 'Authentication required' };
  ...
  // Bulk change status for students (protected)
  bulkChangeStudentStatus: async (payload: { action: 'activate' | 'deactivate', mode: 'all' | 'selected', studentIds: number[] }): Promise<ApiResponse<BulkOperationResult>> => {
    try {
      const token = getToken();
      if (!token) {
        return { error: 'Authentication required' };
      ...
      // Convert studentIds to userIds for API compatibility
      const apiPayload = {
        action: payload.action,
        mode: payload.mode,
        userIds: payload.studentIds
      };
      
      const response = await fetch(`${API_BASE_URL}/students/bulk/status`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  // Bulk delete students (protected)
  bulkDeleteStudents: async (payload: { mode: 'all' | 'selected', studentIds: number[] }): Promise<ApiResponse<BulkOperationResult>> => {
    try {
      const token = getToken();
      if (!token) {
        return { error: 'Authentication required' };
  ...
  deleteStudent: async (id: number): Promise<ApiResponse<any>> => {
    try {
      const token = getToken();
      if (!token) {
        return { error: 'Authentication required' };
  ...
  // Import students from Excel/CSV file (protected)
  importStudents: async (formData: FormData, signal?: AbortSignal): Promise<ApiResponse<any>> => {
    try {
      const token = getToken();
      if (!token) {
        return { error: 'Authentication required' };
      ...
      if (!file) {
        return { error: 'No file provided for import' };
      }
      
      // Add column mapping to ensure proper field mapping (match template headers to database fields)
      ...
      console.log("FormData contents for import:");
      for (const pair of (formData as any).entries()) {
        console.log(`- ${pair[0]}: ${pair[1] instanceof File ? `File (${pair[1].name}, ${pair[1].size} bytes)` : pair[1]}`);
      }
      
      // Use the dedicated students import endpoint
      console.log("Sending import request to:", `${API_BASE_URL}/students/import`);
      ...
      if (!response.ok) {
        try {
          const errorData = await response.json();
          console.error("Import error details:", JSON.stringify(errorData, null, 2));
          ...
          const errorText = await response.text();
          console.error("Import error raw text:", errorText);
          return { error: `Server error ${response.status}: ${errorText.substring(0, 100)}...` };
        }
      }
  ...
  // Export students to CSV/Excel (protected)
  exportStudents: async (format: 'csv' | 'excel' = 'csv'): Promise<ApiResponse<any>> => {
    try {
      const token = getToken();
      if (!token) {
        return { error: 'Authentication required' };
      ...
      // Use dedicated students export endpoint
      const response = await fetch(`${API_BASE_URL}/students/export?format=${format}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      try {
        ...
        if (contentDisposition && contentDisposition.includes('filename=')) {
          ...
          // Default filenames with timestamp
          const date = new Date().toISOString().split('T')[0];
          filename = format === 'csv' ? `students-export-${date}.csv` : `students-export-${date}.xlsx`;
        }
  ...
  // Get import template for students (protected)
  getImportTemplate: async (format: 'excel' | 'csv' = 'excel'): Promise<string> => {
    try {
      const token = getToken();
      if (!token) {
      ...
      // For direct file download with JWT authentication
      const response = await fetch(`${API_BASE_URL}/students/import-template?format=${format}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      console.error('Error generating template URL:', error);
      // Return a fallback URL that includes the API endpoint
      // This won't work directly but will be caught by the UI error handling
      return `${API_BASE_URL}/students/import-template?format=${format}`;
    }
  }
};
